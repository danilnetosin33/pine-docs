...
if (lastReversalBarsShort.length > 0) {
  ...
  temp_el.isOutter = temp_el.barHigh < high;
  if (last_bar_index - temp_el.barIndex > barsCloseReversal) {
    lastReversalBarsShort = [];
  } else if (temp_el.barLow > low) {
    bearish_TAR = true;
    lastReversalBarsShort = [];
  }
}
...
if (true || (bearish_TAR && inDateRange && condBarsIgnore)) {
  ...
  arrayStatistics.push(addItem);
  addItem = outputTableType.new(
    (entrySignal = "BearTAR"),
    (barIndex = bar_index),
    (exitPrice = na),
    (timeEnter = time),
    (initPrice = low[1]),
    (id = arrayStatistics.size() + 1)
  );
  lastHigh = high[1];
  countOpened += 1;
}
...
//CLOSED BY SL
//LONG
if (true || (low < lastLow && entryBarindexLong.length > 0)) {
  closedBySL = true;
  if (
...
// SHORT
if (true || (high > lastHigh && entryBarindexShort.length > 0)) {
  closedBySL = true;
  if (
  ...
  entryBarindexLong.forEach((el, index) => {
    if (
      ...
      arrayStatistics[elIndex] = {
        ...
        signalExit: "Bars",
        exitPrice: open,
        profit: countTakeProfitPerTrade(entryPriceLong[index], open, order),
        barIndexExit: bar_index,
      };
      entryPriceLong.splice(index, 1);
      entryBarindexLong.splice(index, 1);
...
if (entryBarindexShort.length > 0 && entryPriceShort.length > 0) {
  entryBarindexShort.forEach((el, index) => {
    if (
      ...
      closedByBars = true;
      totalProfit +=
        countTakeProfitPerTrade(entryPriceShort[index], open, order) * -1;
